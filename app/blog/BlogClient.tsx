...
export default function BlogClient({ initialPosts }: { initialPosts: BlogPost[] }) {
    ...
    return (
        <main className="min-h-screen bg-background font-poppins font-normal">
            ...
            <section className="py-12">
                <div className="max-w-7xl mx-auto px-4 md:px-6">
                    <div className="flex items-center gap-4 mb-12">
                        ...
                        <span className="text-xs font-semibold text-slate-500  tracking-widest">{filteredPosts.length} Articles</span>
                    </div>
            ...
            {activeCategory === 'All' && !searchTerm && (
                <TrendingIntelligence posts={initialPosts} />
            )}
                ...
                <div className="max-w-[1400px] w-full mx-auto px-6 md:px-12 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-24">
                    ...
                    <div className="flex-1 text-center lg:text-left">
                        ...
                        >
                            ...
                            <span className="text-[10px] font-semibold text-white/80  tracking-[0.3em]">Exclusive Intel</span>
                        </motion.div>

                        <h2
                            className="text-4xl md:text-6xl lg:text-7xl font-semibold text-white mb-6 tracking-tighter  leading-[0.9]"
                        >
                        ...
                        >
                            <div className="flex -space-x-2">
                                {['bg-blue-500', 'bg-purple-500', 'bg-cyan-500', 'bg-emerald-500'].map((color, i) => (
                                    <div key={i} className={`w-8 h-8 rounded-full ${color} border-2 border-[#040b1a] flex items-center justify-center text-[8px] text-white font-semibold`}>
                        ...
                        <div className="p-8 md:p-10 rounded-[2rem] bg-white/[0.04] border border-white/10 backdrop-blur-md shadow-2xl relative overflow-hidden group">
                            ...
                            <form className="flex flex-col sm:flex-row gap-3 mb-6 relative z-10" onSubmit={handleSubscribe}>
                                <div className="flex-grow flex flex-col gap-2">
                                    ...
                                    />
                                    {subscribeMessage && (
                                        <p className={`text-xs ml-2 font-semibold ${subscribeStatus === 'success' ? 'text-[#3994fa]' : 'text-red-400'}`}>
                                            {subscribeMessage}
                                        </p>
                                    )}
                                </div>
                                <button
                                    type="submit"
                                    disabled={subscribeStatus === 'loading'}
                                    className="h-14 shrink-0 px-8 bg-gradient-to-r from-[#3994fa] to-[#004aad] text-white rounded-xl font-semibold  tracking-widest hover:opacity-90 hover:scale-[1.02] transition-all active:scale-95 whitespace-nowrap shadow-lg shadow-[#3994fa]/25 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4 relative z-10 px-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-semibold text-white/40  tracking-widest">No Noise</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-full bg-[#3994fa]/15 flex items-center justify-center">
                                        <Trophy className="w-3 h-3 text-[#3994fa]" />
                                    </div>
                                    <span className="text-[10px] font-semibold text-white/40  tracking-widest">VIP Access</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-full bg-[#3994fa]/15 flex items-center justify-center">
                                        <Rocket className="w-3 h-3 text-[#3994fa]" />
                                    </div>
                                    <span className="text-[10px] font-semibold text-white/40  tracking-widest">1-Click Exit</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
            ...
            <section className="py-12 md:py-16 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-background relative overflow-hidden transition-colors duration-300">
                <div className="max-w-4xl mx-auto px-4 md:px-6">
                    <div className="relative rounded-[2rem] overflow-hidden bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 shadow-lg p-6 md:p-10 text-center group">
                        ...
                        >
                            <h2 className="text-3xl md:text-4xl lg:text-5xl font-semibold text-slate-900 dark:text-white mb-4 tracking-tighter  leading-[0.9]">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                                <Link
                                    href="/services"
                                    className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-[#3994fa] to-[#004aad] text-white rounded-xl font-semibold  tracking-widest hover:opacity-90 transition-all hover:scale-105 active:scale-95 shadow-md shadow-[#3994fa]/10"
                                >
                                    View Capabilities
                                </Link>
                                <Link
                                    href="/contact"
                                    className="w-full sm:w-auto px-8 py-4 bg-transparent border-2 border-slate-200 dark:border-white/10 text-slate-700 dark:text-white rounded-xl font-semibold  tracking-widest hover:bg-slate-50 dark:hover:bg-white/5 hover:border-slate-300 dark:hover:border-white/20 transition-all flex items-center justify-center gap-2 active:scale-95"
                                >
